...
def read_data(distances, previous_clustering):
    ...
    df_distances = pd.read_csv(distances, header=None, sep='\t', names=['sample1', 'sample2', 'distance'])
    if previous_clustering:
        ...
        df_previous_clustering = pd.DataFrame(columns=['sample', 'curated_cluster', 'final_cluster'])
...
def construct_new_cluster_name(current_clusters_dict, separator):
    ...
    current_clusters = flatten_list([cluster.split(separator) for cluster in current_cluster_values])
...
def infer_clusters(graph, merged_cluster_separator):
    ...
    for connected_component in nx.connected_components(graph):
        ...
        if len(set_curated_clusters) > 1:
            logging.critical(f"WARNING: Curated clusters {set_curated_clusters} have merged!")
            inferred_cluster = construct_merged_cluster_name(set_curated_clusters, merged_cluster_separator)
        elif len(set_curated_clusters) == 1:
            inferred_cluster = list(set_curated_clusters)[0]
            logging.warning(f"Cluster {inferred_cluster} is curated and not merged with others")
        elif len(set_final_clusters) > 1:
            logging.critical(f"WARNING: Final clusters {set_final_clusters} have merged!")
            inferred_cluster = construct_merged_cluster_name(set_final_clusters, merged_cluster_separator)
        elif len(set_final_clusters) == 1:
            inferred_cluster = list(set_final_clusters)[0]
            logging.info(f"Cluster {inferred_cluster} is known and not merged with others")
        else:
            # should check existing cluster names
            logging.info(f"Creating new cluster name")
            inferred_cluster = construct_new_cluster_name(inferred_cluster_dict, merged_cluster_separator)
            logging.info(f"New cluster name is {inferred_cluster}, for samples {list_nodes}")
...
def create_output(inferred_cluster_dict, df_previous_clustering, output_path):
    ...
    df_out = pd.DataFrame.from_dict(inferred_cluster_dict, orient='index', columns=['inferred_cluster'])
    df_out.reset_index(inplace=True, names=['sample', 'inferred_cluster'])
    
    logging.info(f"Merging to include previous clustering")
    df_out = df_out.merge(df_previous_clustering[['sample', 'curated_cluster']], on='sample', how='left')
    
    # if curated_cluster is NaN, replace with inferred_cluster
    df_out['final_cluster'] = df_out['curated_cluster'].fillna(df_out['inferred_cluster'])
...
@timing
def main(args):
    df_distances, df_previous_clustering = read_data(args.distances, args.previous_clustering)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--previous-clustering', type=Path, help='Path to previous clustering')
    parser.add_argument('--distances', type=Path, help='Path to distances', required=True)
    parser.add_argument('--output', type=Path, help='Path to output', default=sys.stdout)
    parser.add_argument('--threshold', type=float, help='Threshold to consider two isolates part of the same cluster')
    parser.add_argument('--merged-cluster-separator', type=str, help='Separator for merged clusters', default='|')
    parser.add_argument('--log', type=Path, help='Path to log file', default='cluster.log')
